    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            ...
            showDot.post {
                ...
                val timeTaken =
                    (drawTime - visibilitySetTime) / 1000000 // Convert from nano to milliseconds
        ...
        stopDot.setOnClickListener {
            ...
            showDot.post {
                ...
                val timeTaken =
                    (disappearTime - invisibilitySetTime) / 1000000 // Convert from nano to milliseconds
                Log.d(
                    "VisibilityTiming",
                    "Time taken for reddot to disappear: ${timeTaken}ms"
                ) // Measure the time when the view is actually gone by finding the latency
                invisibilityTimeTextView.text = "Time to disappear: ${timeTaken}ms"
            }

            // Calculate and display the reaction time
            val reactionTime =
                (invisibilitySetTime - dotVisibleTime) / 1000000 // Convert to milliseconds
